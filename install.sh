...
if [[ $distro = "debian" ]]; then
   ...
   arch-chroot /mnt dnf install -y --setopt=install_weak_deps=False --setopt=keepcache=True $grub passwd linux-firmware efibootmgr os-prober btrfs-progs dosfstools $cpu git $virtual
   #*microcode?*
...
if [[ $distro = "fedora" ]]; then arch-chroot /media/loop dnf --installroot=/mnt install -y kernel; fi
