...
install_packages ()
{
  ...
  arch-chroot /mnt zypper -n --gpg-auto-import-keys in -f --replacefiles filesystem coreutils gawk kernel-default busybox-adduser glibc-locale rpm-config-SUSE rpm libsolv-tools libzypp zypper $GRUB os-prober ucode-$CPU btrfsprogs dosfstools cryptsetup sudo NetworkManager fish $VIRTUAL || \
  return 1
